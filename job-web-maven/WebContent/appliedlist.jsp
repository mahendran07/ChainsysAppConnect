	...
	<div class="collapse navbar-collapse" id="navbar">
		<ul class="navbar-nav mr-auto">
			<li class="nav-item "><a class="nav-link" href="recruitersform.jsp">Post
					Vacancy </a></li>
			<li1 class="nav-item active"> <a class="nav-link"
		...
		<div class="card-body">
			<table class="table table-hover">
				...
				<h4>${NOTFOUND}</h4></td></tr>
					<c:forEach var="appliedlist" items="${APPLIEDLIST}">
						<tr>
							<td>${appliedlist.username}</td>
							<td>${appliedlist.email}</td>
							<td>${appliedlist.company_name}</td>
							<td>${appliedlist.vaccancy_position}</td>
							<td>${appliedlist.skill_required}</td>
							<td>${appliedlist.location}</td>
							<td>
								<form method="post" action="AdminSendMailServlet">
									<button type="submit"
										class="btn btn-success waves-effect btn-sm" id="btn"
										name="Username" value="${appliedlist.email}">
										Sendmail</button>
								</form>
					</c:forEach>
				</tbody>
